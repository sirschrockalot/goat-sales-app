 ...
 */
export const ALL_ACOUSTIC_TEXTURES = [
    ...FILLERS,
    ...ACOUSTIC_CUES,
    ...TRANSITIONS,
];
 ...
 */
export function getTexturesByContext(context) {
    return ALL_ACOUSTIC_TEXTURES.filter((texture) => texture.context.some((c) => c.includes(context.toLowerCase())));
}
/**
 * Get textures by type
 */
export function getTexturesByType(type) {
    return ALL_ACOUSTIC_TEXTURES.filter((texture) => texture.type === type);
}
/**
 * Get random texture for a context
 */
export function getRandomTextureForContext(context) {
    const textures = getTexturesByContext(context);
    if (textures.length === 0)
        return null;
    // Weight by frequency
    const weightedTextures = [];
 ...
 */
export async function getTextureInjectionProbability() {
    // Default probability
    let probability = 0.3;
 ...
 */
export function injectTextures(text, context, probability = 0.3) {
 ...
 */
export function stripTextures(text) {
    let cleaned = text;
 ...
 */
export function getTextureTagsAsString() {
