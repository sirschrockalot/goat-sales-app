...
function redactSensitiveData(obj) {
    if (typeof obj === 'string') {
        ...
        redacted = redacted.replace(/(?:api[_-]?key|token|secret|password)[=:]\s*([^\s,}]+)/gi, '[REDACTED]');
        return redacted;
    }
 ...
 */
const devFormat = format.combine(format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), format.errors({ stack: true }), format.colorize(), format.printf(({ level, message, timestamp, ...metadata }) => {
 ...
 */
const prodFormat = format.combine(format.timestamp(), format.errors({ stack: true }), format.json(), format((info) => {
