...
export interface EnvironmentConfig {
  ...
  openai: {
    apiKey: string;
    closerModel: string; // GPT-4o for Closer
    sellerModel: string; // GPT-4o-mini for Sellers/Referees
  };
  vapi: {
    apiKey: string;
    secretKey: string;
    ...
    sttModel: string; // nova-2-general
  };
  elevenlabs?: {
    apiKey: string;
    model: string;
 ...
 * Uses serviceRoleKey for admin operations, falls back to anonKey for read-only
 */
export function getSupabaseClientForEnv(env: Environment): SupabaseClient {
...
export function validateEnvironmentConfig(config: EnvironmentConfig): void {
  ...
  if (errors.length > 0) {
    throw new Error(
      `Environment configuration errors (${config.environment}):\n${errors.map((e) => `  - ${e}`).join('\n')}`
    );
  }
}
 ...
 */
export function assertEnvironment(expectedEnv: Environment): void {
  const config = getEnvironmentConfig();
  if (config.environment !== expectedEnv) {
    throw new Error(
      `⚠️  ENVIRONMENT MISMATCH: Expected ${expectedEnv}, but current environment is ${config.environment}.\n` +
...
export function assertNotProduction(): void {
  ...
  if (config.isProduction) {
    throw new Error(
      '⚠️  SAFETY CHECK FAILED: Attempting to run sandbox/local operation in PRODUCTION mode.\n' +
 ...
 */
export function getOpenAIModel(role: 'closer' | 'seller' | 'referee'): string {
  const config = getEnvironmentConfig();
